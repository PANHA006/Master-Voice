...
សូមអរគុណសម្រាប់ការប្រើប្រាស់សេវាកម្មរបស់យើង។`
    ...
    async function init() {
        elements = {
            ...
            openEditorBtn: document.getElementById('ttsOpenEditorBtn'),
            ...
            autoBreakToggle: document.getElementById('ttsAutoBreakToggle'),
            autoBreakStatusText: document.getElementById('ttsAutoBreakStatusText')
        };
    ...
    function updateTimestampTimings(speedRate, actualMediaDuration) {
        ...
        // Check if rawLinesData already has valid timestamps generated by the server VTT
        const hasServerTimestamps = rawLinesData.some(item => item.seconds > 0 || (item.timestamp && item.timestamp !== '00:00'));

        if (hasServerTimestamps) {
            ttsParsedLines = rawLinesData.map((item) => {
                const timeStr = item.timestamp || formatTime(item.seconds || 0);
                return {
                    text: item.text,
                    mediaSeconds: item.seconds || 0,
                    seconds: item.seconds || 0,
                    timestamp: timeStr,
                    formattedLine: item.formattedLine || `[${timeStr}] ${item.text}`
                };
            });
        } else {
            // Proportional distribution based on actual media duration
            ...
            ttsParsedLines = rawLinesData.map((item) => {
                ...
                const timeStr = formatTime(curMediaSec);

                const lineObj = {
                    text: item.text,
                    mediaSeconds: curMediaSec,
                    seconds: Math.round(curMediaSec * 100) / 100,
                    timestamp: timeStr,
    ...
    function setupEventListeners() {
        ...
        setupAutoBreak(elements.scriptInput, updateCharCount, () => isAutoBreakEnabled);
        ...
        if (elements.openEditorBtn) {
            elements.openEditorBtn.addEventListener('click', () => {
                ...
                if (typeof window.loadAudioToVoiceEditor === 'function') {
                    window.loadAudioToVoiceEditor(currentAudioDataUri, currentFileName || `TTS_${Date.now()}.mp3`);
    ...
    async function synthesize() {
        ...
        try {
            ...
            currentAudioDataUri = data.audioUrl || data.audioDataUri;
            currentFileName = data.fileName || (data.audioUrl ? data.audioUrl.split('/').pop() : `tts-${Date.now()}.mp3`);
            elements.audioElement.src = currentAudioDataUri;
            elements.audioElement.playbackRate = 1.0;
            ...
            rawLinesData = data.timestamps || data.rawLines || [];
            updateTimestampTimings(1.0, data.duration);

            elements.audioElement.onloadedmetadata = () => {
                const realDuration = elements.audioElement.duration || data.duration || 0;
                elements.totalDuration.textContent = formatTime(realDuration);

                // Calibrate timestamps to exact real audio duration
                updateTimestampTimings(1.0, realDuration);
    ...
    function onTimeUpdate() {
        ...
        elements.currentTime.textContent = formatTime(curMedia);
        ...
        ttsParsedLines.forEach((item, idx) => {
            const checkTime = item.seconds !== undefined ? item.seconds : (item.mediaSeconds !== undefined ? item.mediaSeconds : 0);
